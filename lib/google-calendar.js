...
export async function listUpcomingEvents(daysAhead = 7) {
  ...
  if (!calendarId) return { ok: false, events: [], error: 'GOOGLE_CALENDAR_ID not configured' };

  const now = new Date();
  const max = new Date(now);
  max.setDate(max.getDate() + daysAhead);

  // Use simple API key if available (works for public calendars)
  if (apiKey) {
    try {
      ...
      if (!res.ok) return { ok: false, events: [], error: data.error?.message || 'Failed to fetch events' };
  ...
  const { client, calendarId: saCalendarId, error } = getCalendarClient();
  if (error) return { ok: false, events: [], error };

  try {
    const res = await client.events.list({
      calendarId: saCalendarId,
      timeMin: now.toISOString(),
      timeMax: max.toISOString(),
      singleEvents: true,
      orderBy: 'startTime',
