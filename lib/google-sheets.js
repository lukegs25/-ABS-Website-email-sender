...
export async function getUpcomingSheetEvents(daysAhead = 30) {
  ...
  // Use Mountain Time midnight so past events in MT are correctly excluded
  // even when the server runs in UTC
  const nowMT = new Date(
    new Date().toLocaleString("en-US", { timeZone: "America/Denver" })
  );
  nowMT.setHours(0, 0, 0, 0);

  const maxMT = new Date(nowMT);
  maxMT.setDate(maxMT.getDate() + daysAhead);

  return events.filter((e) => {
    // Compare the event's local date (YYYY-MM-DD) against today in Mountain Time
    const eventDate = e.date || new Date(e.start).toISOString().slice(0, 10);
    const todayStr = `${nowMT.getFullYear()}-${String(nowMT.getMonth() + 1).padStart(2, "0")}-${String(nowMT.getDate()).padStart(2, "0")}`;
    const maxStr = `${maxMT.getFullYear()}-${String(maxMT.getMonth() + 1).padStart(2, "0")}-${String(maxMT.getDate()).padStart(2, "0")}`;
